...
public class CreateContact extends AppCompatActivity implements View.OnClickListener {
    ...
    EditText etName, etNumberPhone, etWeb, etMap;
    ...
    public void onClick(View view) {
        if (etName.getText().toString().isEmpty() || etNumberPhone.getText().toString().isEmpty() ||
                etWeb.getText().toString().isEmpty() || etMap.getText().toString().isEmpty()) {
            Toast.makeText(this, "Please enter all fields!", Toast.LENGTH_SHORT).show();
        } else {
            Intent intent = new Intent();
            intent.putExtra("name",etName.getText().toString().trim());
            intent.putExtra("numberPhone",etNumberPhone.getText().toString().trim());
